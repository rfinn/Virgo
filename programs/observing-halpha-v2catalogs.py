...
######  READ IN DATA TABLES 
######
######  switching to v2 catalogs as of spring 2022
######  but keeping v1 VFID names so I don't
######  have to mess with offsets
##################################################
...
def plot_positions(plotsingle=True, flag = None,plotha=True):
    #if plotsingle:
    #    plt.figure()
    #flag = need_obs
...
def make_plot_2018(plotsingle=True, flag=None):
    if plotsingle:
        plt.figure(figsize=(9,4))
    add_nsa()
    plot_positions(plotsingle=plotsingle,flag=None)
    #add_detections()
    add_pointings()
    add_CJpoints()
...
def finding_chart(npointing,delta_image = .25,offset_ra=0.,offset_dec=0.,plotsingle=True,ING=False,MLO=False,KPNO=False,BOK=False):
    ...
    print('i={}, pointing {}, vr={}, ngal = {}'.format(i,pointing_id[i],pointing_vr[i],np.sum(gals)))
    gindex=np.arange(len(v.main['RA']))[gals]
    galnames = v.main['VFID_V1'][gindex]
    ...
    for j in gindex:
        ...
        plt.gca().add_artist(rect)
        s='{}\n vr={:.0f}'.format(v.main['VFID_V1'][j],v.main['vr'][j])
...
def airmass_plotsv2(pointing_id,KPNO=False,ING=False,MLO=False):
    ...
    if KPNO:
        ...
        # April 2022 Run
        start_time = Time('2021-04-25 01:00:00') # UTC time, so 1:00 UTC = 6 pm AZ mountain time
        end_time = Time('2021-04-25 14:00:00')
    ...
    nplots = int(sum(obs_flag)/8.)
    if (sum(obs_flag)/8.) > nplots:
        remainder = sum(obs_flag) - 8*nplots
        nplots += 1
        partial = True
...
for vf in observed_2021Mar:
    ...
    if sum(flag) > 0:
        ...
        #print(i,vf,': before setting flag, obs_flag = ',obs_flag[i])
        obs_flag[i] = False
    else:
...
print('after removing targets from bok mar run: ',sum(obs_flag))
...
print('after removing targets from bok apr run: ',sum(obs_flag))
...
#obs_flag = COsample & ~ha_obs #& (jmass.MSTAR_50 > 8.5) #& (jmass.MSTAR_50 < 10.) #& (nsa.SERSIC_BA > 0.2)

obs_flag = (COsample & ~ha_obs) | extra_targ_flag


if INGrun:
    obs_flag = obs_flag & INTvflag
'''
...
if moretargets == True:
    ...
    obs_flag = more_targets_flag

########################################
###### COLOR VARIABLE FOR SCATTER PLOTS
########################################
...
########################################
###### KEEPING ONLY REQUIRED INFO FOR
###### FOR GALAXIES THAT MEET CRITERIA
########################################

pointing_ra = v.main['RA'][obs_flag]
pointing_dec = v.main['DEC'][obs_flag]
pointing_id = v.main['VFID_V1'][obs_flag]
...
######################################################
###### OFFSETS TO GET MULTIPLE GALAXIES WITHIN FOV 
...
# offset all 90prime targets to get galaxies closer to good corner of lower ccd
if BOKrun:
