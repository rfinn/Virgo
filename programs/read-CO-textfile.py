...
gitpath = '/Users/rfinn/github/Virgo/'
...
infile = gitpath+'tables/all-virgo.2018Jan12.txt'
...
for line in inf1:
    ...
    if len(line) < 1: # skip blank lines
        continue
    # removing filament part for now
    # latest file (Jan 2018) does not have objects sorted by filament
    if line.startswith('#'):
        continue
...
co10 = np.recfromcsv(gitpath+'tables/all-co10.tab',delimiter='')
...
for j in range(len(co10['source'])):    
    ...
    if not(matchflag_co10[i]):
        print('no CO 1-0 match for ',co10['source'][j])
...
co21 = np.recfromcsv(gitpath+'tables/all-co21.tab',delimiter='')
...
for j in range(len(co21['source'])):    
    ...
    if not(matchflag_co21[i]):
        print('no CO 2-1 match for ',co21['source'][j])
...
# removed filament from output file
...
cols = fits.ColDefs([col1,col2,col3,col4,col5,col6,col7,col8,col9,col10,col12,col13,col14,col15,col16,col17,col18,col19,col20,col21,col22,col23,col24,col25,col26,col27,col28,col29])

tbhdu = fits.BinTableHDU.from_columns(cols)

date_stamp = time.strftime("%Y%b%d")
tbhdu.writeto(gitpath+'tables/CO-MasterFile-'+date_stamp+'.fits',overwrite=True)
