...
if mypath.find('rfinn') > -1:
    ...
    gitpath='/Users/kellywhalen/Github/Virgo'
...
NSA_file= gitpath + '/nsa_v0_1_2.fits'
...
# difference (aka Virgo-centric cartesian supergalactic coordinates)
DSGX = SGX - SGX_Virgo
DSGY = SGY - SGY_Virgo
DSGZ = SGZ - SGZ_Virgo
...
kim_flag =  kim_raflag & kim_decflag & dist_flag & kim_vflag 

d_split = 16.
#In front of cluster
SGYvirgo_flag = (DSGY > 4. * u.Mpc) & (DSGY < d_split * u.Mpc)
SGYback_flag = (DSGY > d_split * u.Mpc) & (DSGY < 27. * u.Mpc)
...
def plotxzplane():
    ...
    plt.scatter(DSGX[kim_flag & SGYvirgo_flag],DSGZ[kim_flag & SGYvirgo_flag],alpha=.5,c=DSGY[kim_flag & SGYvirgo_flag])
    plt.colorbar(fraction=.08,label='SGY (Mpc)')
    plt.subplots_adjust(bottom=.15)
    #plt.axis([-15,10,-10,20])
    plt.xlabel('$\Delta DSGX \ (Mpc)$')
    plt.ylabel('$\Delta DSGZ \ (Mpc)$')
...
def plotv3d():
    ...
    ax.scatter(DSGX[kim_flag & SGYvirgo], DSGY[kim_flag & SGYvirgo], DSGZ[kim_flag & SGYvirgo])
    plt.title('Region Around Virgo Cluster')
    plt.xlabel('DSGX (Mpc)')
    plt.ylabel('DSGY (Mpc)')
    #plt.zlabel('SGZ (Mpc)')
